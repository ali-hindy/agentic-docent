...
class ImageRetrieval:
    def __init__(
        ...
        if vector_db_path is None:
            self.vector_db_path = (
                f"vector_database_{embedding_type.lower()}_{data_name}.npy"
            )
        else:
    ...
    def get_or_create_image_embeddings(self):
        if os.path.exists(self.vector_db_path):
            print(
                f"Loading {self.embedding_type} embeddings from {self.vector_db_path}..."
            )
            ...
            print(
                f"Generating {self.embedding_type} embeddings and saving to {self.vector_db_path}..."
            )
    ...
    def extract_image_embeddings(self):
        ...
        if self.embedding_type == "ResNet":
            ...
            for i in tqdm(range(0, len(image_files), batch_size)):
                ...
                batch_images = [
                    Image.open(os.path.join(self.dataset_dir, f)).convert("RGB")
    ...
    def build_similarity_index(self):
        ...
        print(
            f"Normalized embedding norms: min={check_norms.min():.6f}, max={check_norms.max():.6f}"
        )
...
def evaluate_retrieval_accuracy(retriever: ImageRetrieval, test_dir: str) -> dict:
    ...
    db_files = [
        f
        for f in os.listdir(retriever.dataset_dir)
        if f.endswith((".jpg", ".jpeg", ".png"))
    ]
    ...
    for test_file in tqdm(test_files):
        ...
        if not retrieved_bases[0] == query_base:
            ...
            print(
                f"Top {retriever.k} scores: {[f'{score:.3f}' for _, _, score in similar_images[:retriever.k]]}"
            )
    ...
    metrics = {
        ...
        "failure_cases": failure_cases[
            :10
        ],  # Only keep first 10 failures for readability
    }
...
def print_evaluation_results(metrics: dict):
    ...
    print(
        f"Average Similarity Score (Correct): {metrics['avg_correct_similarity']:.3f}"
    )
    print(
        f"Average Similarity Score (Incorrect): {metrics['avg_incorrect_similarity']:.3f}"
    )
