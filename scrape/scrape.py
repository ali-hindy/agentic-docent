...
# Helper function to scrape data for each painting
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for painting in jsonResponse["data"]:
                    painting_uris.append(os.path.join(painting["artistUrl"], painting["url"]))
                    save_painting_image(painting, image_dir)

        except Exception as e:
            print(f"Error fetching page URLs: {e}")
    return painting_uris
...
def scrape_wikiart(output_dir):
    ...
    page_urls = scrape_paintings(image_dir)
    
    # Iterate over each URL and save as PDF
    pdf_dir = os.path.join(output_dir, "pdfs")
    for url in page_urls:
        save_page_as_pdf(url, pdf_dir)
