        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for painting in jsonResponse["data"]:
                    painting_data[
                        os.path.join(painting["artistUrl"], painting["url"])
                    ] = {
                        "artist": painting["artistName"],
                        "title_of_work": painting["title"],
                        "date_created": painting[
                            "completitionYear"
...
def save_page_as_pdf(uri, output_dir):
    ...
    while True:
        try:
            ...
            doc.copy(doc.pages[0:1]).write_pdf(
                output_path
            )  # For these renders, the third page contains our target data
...
def sample_paintings(data_path, n_samples):
    ...
    sampled_items = np.random.choice(
        list(painting_data.keys()), size=n_samples, replace=False, p=zipf_probs
    )
...
def write(output_dir, painting_data, painting_keys=None):
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        futures = [
            executor.submit(process_painting, uri, painting_data, image_dir, json_dir)
            for uri in painting_keys
        ]
        for future in tqdm(
            as_completed(futures), total=len(painting_keys), desc="Processing paintings"
        ):
            try:
                future.result()
            except Exception as e:
