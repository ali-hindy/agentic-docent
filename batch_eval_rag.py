...
def run_batch_evaluation(image_dir: str, json_dir: str, max_samples: int = 5) -> None:
    ...
    for image_path in tqdm(image_dir.glob("*.jpg")):
        ...
        try:
            similar_image_path, metadata = image_retriever.retrieve_most_similar_image(
                str(image_path)
            )

            vlm_response = metadata  # evaluator.get_vlm_response_rag(metadata)
            ground_truth = evaluator.load_ground_truth(
                str(json_dir / f"{image_path.stem}.json")
            )
...
if __name__ == "__main__":
    run_batch_evaluation(
        image_dir="../data_v3/images", json_dir="../data_v3/json", max_samples=500
    )
