...
def run_batch_evaluation(image_dir: str, json_dir: str, max_samples: int = 5) -> None:
    ...
    docent = DocentPipeline(
        image_dir, json_dir, os.getenv("TOGETHER_API_KEY"), embedding_type="CLIP"
    )
                    ...
                    for field, score in factual_result["field_scores"].items():
                        if score == 0:
                            if field in factual_result["incorrect_fields"]:
                                extracted = factual_result["incorrect_fields"][field][
                                    "extracted"
                                ]
                                ground_truth_value = factual_result["incorrect_fields"][
                                    field
                                ]["ground_truth"]
                            else:
                                extracted = factual_result["extracted_values"].get(
                                    field, "MISSING"
                                )
                            ...
                            results[approach]["error_analysis"][field].append(
                                {
                                    ...
                                    "present_in_text": factual_result[
                                        "incorrect_fields"
                                    ]
                                    .get(field, {})
                                    .get("present_in_text", False),
                                }
                            )
    ...
    for approach in ["docent", "baseline"]:
        ...
        factual_acc = sum(
            val["accuracy"] for val in results[approach]["factual"].values()
        )
        avg_factual_accuracy = (
            factual_acc / len(results[approach]["factual"])
            if results[approach]["factual"]
            else 0
        )

        # Calculate style scores
        style_acc = sum(
            val["overall_style_score"] for val in results[approach]["style"].values()
        )
        avg_style_score = (
            style_acc / len(results[approach]["style"])
            if results[approach]["style"]
            else 0
        )
        ...
        for criterion, analyses in results[approach]["style_analysis"].items():
            criterion_avg = sum(analysis["score"] for analysis in analyses) / len(
                analyses
            )
...
if __name__ == "__main__":
    run_batch_evaluation(
        image_dir="./data_v2/images_handheld", json_dir="./data_v2/json", max_samples=5
    )
